...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.checkoutForm = this.fb.group({
      ...
      AudometricStatusListId: [5, Validators.required],
      Checkout: [true],
  ...
  getCheckoutDetails() {
    ...
    this.audiometricService.getCheckoutDetails(checkoutHistory)
    .pipe(takeUntil(this.onDestroyUnSubscribe))
    .subscribe((checkoutData: any) => {
      if (checkoutData['status'] == 200) {
        this.checkoutDetails = checkoutData['audCheckOut'];
  ...
  checkedTrue() {
    if(((this.checkoutDetails.NoiseExposure === true) && (this.checkoutDetails.MedicalHistory === true) && (this.checkoutDetails.Results === true))) {
  ...
  onSubmit() {
    ...
    if (this.checkoutForm.valid) {
      ...
      this.audiometricService.addEditCheckout(checkoutDataPayLoad)
      .pipe(takeUntil(this.onDestroyUnSubscribe))
